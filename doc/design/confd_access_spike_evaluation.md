...
The ITDE-backed integration test must record the ConfD RPC port configured in
the disposable Docker-DB EXAConf and whether Docker forwards that port to the
host. An unmapped configured port requires a separate ITDE port-exposure
change before an external JSON-RPC integration test can run. The EXAConf
`XMLRPCPort` setting alone does not identify the protocol used internally by
`confd_client` and must not be used to reject JSON-RPC.
...
The ITDE-backed integration test must use ITDE's temporary generated SSH key
and temporary known-hosts file to execute `confd_client db_list -j` through the
forwarded SSH port. The private-key file must have mode `0600`, allowing only
its owner to read it. The test must parse the read-only JSON result and confirm
that a rejected SSH login does not expose private-key material.
